...
function MainPage() {
  ...
  const toggleWindow = (windowName: keyof typeof openWindows) => {
    setOpenWindows((prevState) => ({
      ...prevState,
      [windowName]: !prevState[windowName],
    }));
  };
    ...
    <div>
      <div className="xyCenter">
        <div className="mainAttraction">
          <img src="/otherImages/dave.png" className="me" draggable="false" />{" "}
          <br />
          <span className="header">
            Heyo! I'm <b>Daveberry</b>.
          </span>
        </div>
            ...
            <div key={link.Name}>
              <a href={link.Links} target="_blank" rel="noopener noreferrer">
                <img
                  src={link.Icons}
                  alt={link.Name}
                  className="socialLogo invertLogo"
                  draggable="false"
                />
              </a>
            </div>
      ...
      {openWindows.others && (
        <OthersWindow onClose={() => toggleWindow("others")} />
      )}
    </div>
  );
}
